...
def get_department_stay_class(id: int, num: int, db: Session = Depends(get_db)):
    try:
        data = admin_crud.get_all_student_stay_in_school_more_than_n_days_info_in_class_level(id, num, db)
        return schemas.Response(data=data)
    except Exception as e:
        return schemas.Response(message=str(e), code=500)
...
@app.get("/stu/health/report", response_model=schemas.Response)
def report_stu_health(stu_number: int, db: Session = Depends(get_db)):
    try:
        data = student_crud.get_report_student_health(stu_number, db)
        return schemas.Response(data=data)
    except Exception as e:
        return schemas.Response(message=str(e), code=500)
